...
// NewLRU creates a new LRU cache with the capacity of cap.
func NewLRU[K comparable, V any](cap int) (*LRU[K, V], error) {
	if cap <= 1 {
...
func moveToTop[K comparable, V any](c *LRU[K, V], n *node[K, V]) {
	if n != c.head {
		if n == c.tail {
			c.tail = n.prev
		}
